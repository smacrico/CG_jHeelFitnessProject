...
class HRVAnalytics:
    ...
    def analyze_hrv_trends(self, days: int = 30, include_stats: bool = True) -> Dict:
        ...
        # Group by date and calculate aggregates including std deviation
        grouped = df.groupby('date').agg(
            avg_rmssd=('hrv_rmssd', 'mean'),
            std_rmssd=('hrv_rmssd', 'std'),
            avg_sdnn=('sdnn', 'mean'),
            avg_pnn50=('hrv_pnn50', 'mean'),
            avg_hr=('mean_hr', 'mean'),
            avg_recovery=('recovery', 'mean'),
            session_count=('hrv_rmssd', 'count'),
            min_rmssd=('hrv_rmssd', 'min'),
            max_rmssd=('hrv_rmssd', 'max')
        ).reset_index()
        ...
        results = {
            ...
            "date_range": {
                "start": grouped['date'].min().strftime('%Y-%m-%d'),
                "end": grouped['date'].max().strftime('%Y-%m-%d')
            },
            "dataframe": grouped
        }
        ...
        if len(grouped) > 1:
            ...
            # RMSSD trend
            rmssd_trend = np.polyfit(x, grouped['avg_rmssd'].fillna(0), 1)
            rmssd_r_value = np.corrcoef(x, grouped['avg_rmssd'].fillna(0))[0, 1]
            
            # SDNN trend
            sdnn_trend = np.polyfit(x, grouped['avg_sdnn'].fillna(0), 1)
            sdnn_r_value = np.corrcoef(x, grouped['avg_sdnn'].fillna(0))[0, 1]
            ...
            if include_stats:
                results["statistics"] = self._calculate_trend_statistics(grouped)
    ...
    def _generate_sample_trend_data(self, days: int) -> Dict:
        ...
        df = pd.DataFrame({
            ...
            'std_rmssd': np.abs(np.random.normal(5, 2, days)),
            ...
            'min_rmssd': np.random.normal(40, 5, days),
            'max_rmssd': np.random.normal(60, 5, days)
        })
